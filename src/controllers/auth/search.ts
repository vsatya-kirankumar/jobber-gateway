...
export class Search {
  public async gigById(req: Request, res: Response): Promise<void> {
    const response: AxiosResponse = await authService.getGig(req.params.gigId.toString());

    res.status(StatusCodes.OK).json({ message: response.data.message, gig: response.data.gig });
  }

  public async getGigs(req: Request, res: Response): Promise<void> {
    ...
    const response: AxiosResponse = await authService.getGigs(query, from.toString(), size.toString(), type.toString());
