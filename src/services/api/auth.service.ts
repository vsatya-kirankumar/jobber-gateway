...
import { IAuth } from '@vsatya-kirankumar/jobber-shared';
...
class AuthService {
  ...
  async changePassword(password: string, newPassword: string): Promise<AxiosResponse> {
    const response: AxiosResponse = await axiosAuthInstance.put('/change-password', { password, newPassword });
    return response;
  }
  ...
  async signIn(body: IAuth): Promise<AxiosResponse> {
    ...
    console.log('Inside singIn method: ', response);
    return response;
  }
  ...
  async resendEmail(data: { userId: string; email: string }): Promise<AxiosResponse> {
    const response = await this.axiosService.axios.post('/resend-email', data);
    return response;
  }
